...
for epoch in tqdm(range(EPOCHS)):
    # 执行训练一个轮次
    train_loss = train_epoch(model=denoiser, device=device, train_loader=train_loader, loss_fn=loss_fn, optimizer=optimizer)
    # 执行验证
    val_loss = val_step(model=denoiser, device=device, val_loader=val_loader, loss_fn=loss_fn)
